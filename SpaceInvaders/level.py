...
def load_level(level_name: str, window):
    ...
    column = 1
    for i in range(2):
        game.add_gameobject(Block(screen_width//2 - 140 * (i + 1) - 30,
                                  screen_height - 130, window=window))
        game.add_gameobject(Block(screen_width // 2 + 140 * (i + 1) - 30,
                                  screen_height - 130, window=window))
    for char in level_str:
        if char in enemies_sprite_paths:
            ...
            game.enemy_count += 1
            game.add_gameobject(enemy.Enemy(level.shift_x +
                                            screen_width//5 + 60 * column,
                                            level.shift_y +
                                            screen_height//5 + 60 * row,
                                            50,
                                            50,
                                            window=window,
                                            sprite_path=path))
...
levels = {
    'main_menu': Level('main_menu', '0',
                       'first'),
    'first': Level('first', '3333n2222n1111',
                   'second', shift_y=-100),
    'second': Level('second', '33333n02220n11111',
                    'third', -25, -100),
    'third': Level('third', '0303030n3030303n0202020n1010101',
                   'end', -100, -100),
    'fourth': Level('fourth', '0333330n3333333n0222220n1111111',
                    'end', -100, -100),
}
